...
export namespace Config {
  ...
  export function defaultPlugins(): PluginSpec[] {
  ...
  export function getPluginName(plugin: PluginSpec) {
    const spec = pluginSpecifier(plugin)
    if (isPathPluginSpec(spec)) {
      const pathname = spec.startsWith("file://") ? new URL(spec).pathname : spec
